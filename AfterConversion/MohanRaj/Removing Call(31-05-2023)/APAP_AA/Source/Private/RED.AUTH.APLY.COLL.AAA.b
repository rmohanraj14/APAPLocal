* @ValidationCode : MjoxMjA2MTg3NTI4OkNwMTI1MjoxNjg1NTI3OTk4MjA4OjkxNjM4Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 31 May 2023 15:43:18
...
SUBROUTINE RED.AUTH.APLY.COLL.AAA
...
OPEN.FILES:
    ...
    L.FLDS = 'L.AC.LK.COL.ID':@FM:'L.AA.COL':@VM:'L.AA.COL.VAL':@VM:'L.AA.AV.COL.BAL':@VM:'L.AA.COL.DESC' ;*R22 Auto Code Conversion
...
* PACS00307565 - S
    ...
    Y.AA.DUP.ID = Y.AA.ID
* PACS00307565 - E
    Y.NOM.VAL = R.NEW(COLL.NOMINAL.VALUE)
    Y.OLD.NOM.VAL = R.OLD(COLL.NOMINAL.VALUE)

    IF Y.AA.DUP.ID NE Y.OLD.AA.ID OR Y.NOM.VAL NE Y.OLD.NOM.VAL THEN
...
* PACS00307565 - S
        Y.AA.BAL.VALS  = returnConditions<AA.AMT.LOCAL.REF,Y.COL.AV.POS>  ;* Current AA Balance
...
*
    ...
    LOCATE ID.NEW IN Y.AA.COLL.IDS<1,1> SETTING POS.CL THEN
        ...
        AAA.REQ<AA.ARR.ACT.FIELD.VALUE,1,-1> = R.NEW(COLL.NOMINAL.VALUE)
